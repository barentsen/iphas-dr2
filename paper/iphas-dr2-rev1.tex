...
The goal of this paper is to present the next release, 
 which supersedes the IDR by providing a global re-calibration
and taking the coverage up to 92 per cent of the survey area.}
In this work we
...
\begin{table}
    ...
    \begin{center}
        ...
        %Detector field of view & 11 $\times$ 23 arcmin \\
        Detector scale & 0.33 arcsec/pixel \\        
        Filters & $r$, $i$, \ha \\
        Filter properties & See Table~2 \\
        Exposure times & 30~s ($r$), 10~s ($i$), 120~s (\ha) \\
		Saturation limit & $\sim$13 ($r$), $\sim$12.5 ($i$), $\sim$12.5 (\ha) \\
        Detection limit (5$\sigma$) & 21.3 ($r$), 20.1 ($i$), 20.3 (\ha) \\
...
{ \bf It is these post-processing steps which distinguish the present release
...
The calibration errors can exceed 0.1~mag
...
\caption{IPHAS $(r-\ha,\ r-i)$ colour-colour diagram
         ...
         classified by the pipeline as `bestQualityAllBand'
...
and are hereafter referred to as {\bf `highQualityAllBand'}.
For convenience, the catalogue contains a boolean column
named {\bf \emph{highQualityAllBand}} that directly flags these objects.
...
and 54 per cent are \emph{veryReliable}
...
\item \emph{reliable} sources
\item \emph{veryReliable} sources
\end{enumerate}
...
supplied \emph{reliable} and \emph{veryReliable} quality flags.
...
\begin{table}
    \caption{Reliability values as computed by crossmatching the point 
    ...
    \begin{center}
        ...
        \toprule
        Region ($\ell$, $b$) & Raw & nBands == 3 & $rErr\,<\,0.1$ & $iErr\,<\,0.1$ & \emph{reliable} & \emph{veryReliable} \\
        \midrule
        (149.39, 4.06) & 92.95 & 98.83 & 98.53 & 98.54 & 99.67 & 99.82 \\
        (186.59, -2.50) & 89.99 & 98.82 & 98.34 & 98.44 & 99.36 & 99.57 \\
        (202.70, -1.75) & 89.42 & 98.46 & 97.8 & 98.11 & 99.4 & 99.65 \\
        \bottomrule
        \end{tabular}
    \end{center}
\end{table}
...
\begin{subfigure}[b]{\linewidth}
\centering
\includegraphics[width=0.9\textwidth]{figures/repeatability/repeatability-reliable.pdf}
\caption{}
...
\caption{Photometric repeatability as a function of magnitude
         ...
         and for the \emph{veryReliable} sources alone (panel b).
...
{\bf Moreover, IPHAS provides the much-needed counterpart
to the source catalogues being provided by the recent infrared surveys.
For example, amongst the 1810 high-quality sources considered in Fig.~\ref{fig:emitters},
1623 objects (90\%) have a counterpart in the Spitzer catalogue,
and 371 objects (20\%) have a counterpart in the 
