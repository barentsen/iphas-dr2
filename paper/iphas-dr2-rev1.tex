...
\begin{table}
    \caption{Overview of the key survey properties.}
    \label{tbl:survey}
    \begin{center}
        \begin{tabular}{ll}
        \toprule
        ...
        Exposure time & 30~s ($r$), 10~s ($i$), 120~s (\ha) \\
        Magnitude limit (5$\sigma$, Vega) & 21.3 ($r$), 20.1 ($i$), 20.3 (\ha) \\
        PSF FWHM (median) & 1.1\arcsec\ ($r$), 1.0\arcsec\ ($i$), 1.1\arcsec\ (\ha) \\
        Survey area & $\sim 1860$~deg$^2$ \\
        Footprint limits & $ -5\degr < b < +5\degr,\ 29\degr < \ell < 215\degr$ \\
        \bottomrule
        \end{tabular}
	\end{center}
\end{table}
