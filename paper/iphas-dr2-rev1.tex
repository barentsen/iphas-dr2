...
{ \bf
Source detection and parametrisation is then carried out
...
\item Finally, the parametrisation of overlapping objects
is refined by simultaneous fitting of
soft-edged apertures to each blend,
which is essentially identical 
to multiple "profile" fitting.
\end{enumerate}
A discussion on each of these steps
can be found in \citep{Irwin1985,Irwin1997}.
We note that the parametrisation of single, unconfused objects
is naturally more reliable than that of
objects which had to be deblended.
In \S\ref{sec:catalogue} we will explain
how blended sources are flagged in the catalogue. 


%The sentence about deblending could be misleading since the objects are
%detected using our standard deblending algorithm while the aperture flux
%estimates for overlapping images are computed by simultaneous fitting of
%soft-edged apertures.  During this multiple "profile" fitting the sky
%level is based on the interpolated global detector-level fit
...
when its flux in a 2\farcs 3-diameter aperture
is five times the level of the noise in the sky background.
The sky noise was estimated using a robust MAD estimator 
for noise scaled to equivalent Gaussian rms value, ie. = MAD x 1.48,
