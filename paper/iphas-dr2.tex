...
detection tables into a more reliable source catalogue which
...
% Stating the obvious ...a new insertion? which is a proxy for the image quality
...
% slipped in earlier (the WFC pixel scale is 0.33~arcsec/px).
In \S\ref{sec:catalogue} we will explain
...
Despite the fact that all the fields included in the diagram
have been quality-approved,
the main locus of stars is poorly defined
due to the presence of calibration errors,
which regularly exceed 0.1~mag.
...
\label{fig:ccd_after}
\end{subfigure}
\caption{IPHAS $(r-\ha,\ r-i)$ colour-colour diagram for an area of 400~deg$^2$, plotted before (panel a) and after (panel b)
re-calibration.
Both figures were created by combining the stars detected
across the entire set of 2\,801 quality-approved fields which are located
towards the Galactic anti-centre  ($160\degr < \ell < 200\degr$).
...
We have included all stars which are brighter than $r<18$
and are classified as `very reliable'
(these criteria will be explained in \S\ref{sec:qualitycriteria}).}
\end{figure}

The calibration variations need to be corrected for 
...
they can accumulate during the calibration process, causing artificial zeropoint gradients 
...
%The private communication from Hended is no longer needed :-)
...
%I wonder if the next bit is stating the too-obvious?
...
corrected for the flux lost outside of the applied aperture,
...
%In turn, this has meant that it has not proved necessary 
...
(columns \emph{iXi}, \emph{iEta}, \emph{haXi}, \emph{haEta}), or simply to examine
light centre differences.
%I think of Dimitri Purbaix here.
...
%GEERT.  Is the above too extreme a warning?  Is the r band the 'master', to which the given source 
%position refers, when available, and hence is the natural choice for astrometric purposes?
%The caution applies if you mis-associate the position with e.g. an i source, without
%importing the offsets.  Or do you really believe no position is better than 1 arcsec because
%we don't know which band it refers to.
...
which is most likely explained by the fact that $i$ is least
...
up in $i$ alone, while faint objects with extremely strong Balmer emission
...
but certainly not all. Users of the data are encouraged to revise 
the quality criteria according to their needs.
...
For example, we find a significant drop in source count near the constellations 
...
density.  In headline numbers, only 11\% of the sources are blended
...
and applying the 14\% reduction recommended by \cite{Schlafly2011}.
...
%Sale2014 needs to be 'Sale S. E., et al., 2014, MNRAS, submitted' in the bibliography
...
examine options to further improve the global calibration, with a view to tightening the accuracy to
