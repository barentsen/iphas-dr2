...
to bring 
%the vast majority - exaggerating a bit? 
most of the survey 
...
%of minimising the magnitude differences between overlapping frames 
...
(GEERT: the rest of the sentence looks very odd to me -- I can't rephrase
because I'm not sure what you are trying to say) without removing more 
than 80\%
...
stricter criteria remove large numbers
...
42 million candidate sources are added to the 109 
million satisfying the tighter error bound (GEERT - is this what you mean?).
...
$1.06\pm0.12$ for $(r-i)$ and $0.44\pm0.03$ for $(r-\ha)$. The
fainter limiting magnitude in $r$ is hence compensated
by the fact that most sources have 
...
counted in 1-by-10~deg$^2$ constant-longitude strips,
...
real variations are also apparent on smaller scales.
...
and their photometry is set out in Table~\ref{tbl:emitters}.
...
Two of our candidates, x and y in Tab.~\ref{tab:emitters}, have recently 
been identified
...
i.e. they are likely to be Class II YSOs exhibiting a mild infrared excess.
...
when the extinction is not too high ($A_V < 10$).
...
Schmidt H$\alpha$ Survey \citep[SHS,][][Parker05], the last of the 
