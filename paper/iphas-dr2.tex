...
$^{9}$University College London, Department of Physics \& Astronomy, 
Gower Street, London WC1E 6BT, U.K.\\
$^{10}$Harvard-Smithsonian Center for Astrophysics, 60 Garden Street, 
Cambridge, MA 02138, U.S.A. \\
$^{11}$Th\"uringer Landessternwarte, Sternwarte 5, D-07778, Tautenburg, Germany. \\
$^{12}$Observatorio Astron\'omico, Universidad de Valencia,
Catedr\'atico Jos\'e Beltr\'an 2, 46980 Paterna, Spain.\\
$^{13}$Department of Physics, University of Warwick, Gibbet Hill Road, Coventry, CV4 7AL, U.K.\\
$^{14}$Joint ALMA Observatory, Alonso de Córdova 3107, Vitacura 763-0355, Santiago, Chile. \\
$^{15}$Jodrell Bank Centre for Astrophysics, School of Physics \& Astronomy, University of Manchester, Manchester M13 9PL, U.K.\\
%$^{16}$School of Physics \& Astronomy, University of Southampton, Southampton, SO17 1BJ, U.K.\\
%$^{17}$European Space Astronomy Centre (ESAC), Villafranca del Castillo, Villanueva de la Canada, E-28692 Madrid, Spain.\\
%$^{18}$School of Physics, Bristol University, Tyndall Avenue, Bristol, BS8 1TL, U.K.\\
%$^{19}$School of Physics, University of Exeter, Stocker Road, Exeter, EX4 4QL, U.K.\\
...
%$^{22}$Research Centre for Astronomy, Astrophysics and Astrophotonics, Macquarie University, Sydney, NSW 2109 Australia.\\
%$^{23}$Instituto de Astonom\'ia y Meteorolog\'ia, Departamento de F\'isica, CUCEI, Universidad de Guadalajara, Mexico.\\
%$^{24}$Instituut voor Sterrenkunde, K.U. Leuven, Celestijnenlaan 200D, B-3001 Leuven, Belgium.\\
%$^{25}$Max-Planck-Institut f\"ur extraterrestrische Physik (MPE), Giessenbachstrasse 1, D-85748, Garching bei M\"unchen, Germany. \\
%$^{26}$Department of Physics, Blackett Laboratory, Imperial College London, Prince Consort Road, London, SW7 2AZ, U.K.\\
%$^{27}$Centro de Estudios de F\'isica del Cosmos de Arag\'on, Plaza San Juan 1, Planta 2, Teruel, 44001, Spain. \\
%$^{28}$Armagh Observatory, College Hill, Armagh, Northern Ireland, BT61 9DG, U.K.\\
$^{16-28}$ (affiliations 16 to 28 are given in the source file)
}
...
\begin{figure*}
        ...
        The dark vertical strip near $\ell \sim$125\degr\ 
...
at the default detection level of 1.25\,$\sigma$.
...
is also explained by the shorter $r$-band exposure time
that was used in the first year of data-taking.
...
\emph{across the detectors} exceeded $e > 0.3$.
...
we have decided to accept data obtained with seeing up to 2.5~arcsec.
...
We rejected field pairs in which more than 2 per cent of the stars 
showed an inconsistent measurement at the level of 0.2~mag,
or more than 26 per cent were inconsistent at the level of 0.1~mag.
...
for 14\,115 out of the 15\,270 fields (92 per cent).
...
are detected in all three bands (72 per cent),
30 million are detected in two bands (14 per cent),
and 31 million entries are single-band detections (14 per cent).
...
Moreover, late-type stars tend to be intrinsically brighter in $i$.
...
\begin{table}
    ...
    \label{tbl:detections}
        ...
        \toprule
        Detection & N & \%($nObs>1$) &
        $\overline{pStar}$ & $\overline{(r-i)}$ & $\overline{(r-\ha)}$ \\
        \midrule
        $r$, $i$, \ha & 159 (73\%) & 91\% & 0.82 & 1.03 & 0.44  \\
        $r$, $i$ (not \ha) & 25 (11\%) & 77\% & 0.70 &  1.26 & - \\
        $i$, \ha\ (not $r$) & 3 (1\%) & 73\% & 0.49 &  & \\
        $r$, \ha\ (not $i$) & 2 (1\%) & 65\% & 0.53 & - & 0.58 \\
        only $i$ & 15 (7\%) & 43\% & 0.44 \\
        only $r$ & 9 (4\%) & 27\% & 0.42 \\
        only \ha & 6 (3\%) & 12\% & 0.11 \\
        \midrule
...
\begin{figure*}
    ...
    bins with 10 to 20 objects are orange,
    and bins with 20 to 25 objects are yellow.
...
The catalogue is made available on our website (www.iphas.org)
as a collection of binary FITS tables,
each covering a $5\degr\times5\degr$ tile
of the survey footprint.
The catalogue can be queried in two ways.
First, it is made available through the
Vizier on-line service (http://vizier.u-strasbg.fr)
where it can be queried using their web interface
or using Virtual Observatory (VO) protocols.
Second, on our website we provide a script
to ingest the binary FITS tables into a
{\sc PostgreSQL} database on a user's computer.

The website also provides access to the pipeline-processed
imaging data, which we have updated to include
a new keyword {\sc photzp},
which contains the re-calibrated zeropoint.
It can be used to convert the number counts $DN$
-- which are the pixel values -- 
...
\end{split}
\end{equation}
That is, the {\sc photzp} has been computed such that it
...
To convert the magnitude $m$ into fluxes,
we list in Table~\ref{tbl:flux} something. 
...
Users of the data are reminded to cite both \citet{Drew2005}
and this paper.
