...
$^{15}$AAVSO, 49 Bay State Road, Cambridge, MA 02138, USA.\\
...
Amongst these, 159 million are detected 
in all three bands (73 per cent),
30 million are detected in two bands (14 per cent),
and the remaining 30 million are single-band detections.
The detailed breakdown is presented in Table~\ref{tbl:detections},
along with the fraction of `confirmed' sources,
hich we define
as objects which were detected in more than
one field pointing (usually the alternative pointing of a field pair).
Sources detected in all three 
(Table~\ref{tbl:detections}).
Half of the single-band detections were made in the $i$-band.
This is most likely because $i$ is least
...
Table~\ref{tbl:detections} also shows the fraction of the
sources that were `confirmed', w
For the class of 
in more than one field (usually the field observed at the small offset position)

\begin{table}
    \caption{Breakdown of catalogue entries as a function of the band(s) in which the source was detected. We also show the fraction of `confirmed' objects, which we define as sources which were detected in more than one field pointing (usually the field pair partner). Also included are the mean magnitudes for each class.}
...
First, will be made available through the
Vizier on-line service (http://vizier.u-strasbg.fr),
where it can be accessed using a web interface
...
{\sc PostgreSQL} database for users
who wish to mine the data locally.
...
The keyword can be used to convert the number counts $DN$,
which stored in the pixel values,
...
\end{split}
\end{equation}
That is, {\sc photzp} has been computed such that it
absorbs the required corrections for atmospheric extinction,
gain variations, exposure time, and the re-calibration.

A user will only have to apply a correction
for the flux lost outside
