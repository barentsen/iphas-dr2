...
(defined here as $160\degr < \ell < 200\degr$).
...
after our re-calibration procedure.
...
\caption{IPHAS $(r-\ha,\ r-i)$ colour-colour diagram
         ...
         The objects seen to fall above the locus of stars
...
in these narrow overlap regions to be insufficient
...
and $0.007\pm0.108$~mag in $i$
(Table~\ref{tbl:offsets_before}).
A total of 4,596 IPHAS fields showed a systematic offset
...
strong zeropoint variations to exist during the night,
...
Having re-calibrated all fields
to the expected APASS accuracy of 3 per cent,
we turned to a different survey,
SDSS Data Release 9 \citep{Ahn2012},
...
with criteria {\sc type = star}, {\sc clean = 1} and {\sc score $>$ 0.7}.}, in much the same way as for APASS,
...
because the tables generated by the CASU pipeline
refer e.g. to source positions in pixel coordinates,
to photometric measures in number counts, etc.,
rather than in directly useful astronomical terms.
...
Photometry is provided based on the 2\farcs 3-diameter circular aperture
...
with reliable photometry.

The brightness of each object as a function of increasing
aperture size is also used by the CASU pipeline to provide
a discrete star/galaxy\footnote{For consistency with the terminology that is widespread in the CASU pipeline and the WSA/VSA archives, extended objects are classified as `galaxies'. This class will flag any type of resolved object, however.}/noise classification flag
...
into the final 98-column source catalogue
...
\emph{veryReliable} category suffers most of
its incompleteness at low Galactic longitudes,
...
from 0.1~mag to 0.2~mag in Table~\ref{tab:reliable},
...
but users are nevertheless encouraged 
...
the average colours for sources in the catalogue are
...
for 219~million unique sources.
To support its exploitation, 
we provide a list of recommended quality criteria
that will permit the selection
of objects with reliable colours from the catalogue.
The closing demonstrations highlight the use of the 
survey's unique $(r-\ha,\ r-i)$ diagram
for characterising stellar populations
and selecting emission-line objects.
More comprehensive applications of IPHAS
can be found in the works of Sale et al (submitted),
which applies DR2 to the problem of 3D extinction mapping,
and of Sabin et al (in preparation),
where the results of a search
...
not meeting the IPHAS minimum quality requirements
are continuing.
We will also examine options to further improve
the global calibration,
perhaps tightening the accuracy to better than 2\%.
...
the most dense fields via the implementation
of PSF fitting in place of aperture photometry.  
Finally, the next catalogue will detail
all the secondary detections
